...
**Renamed from "Tracker" to "Compass"**: this only changed what's user-visible (window title, topbar branding, `productName` in `package.json`, the installer's display name). `package.json`'s `"name": "1cloudhub-tracker"` is deliberately unchanged — see the renaming hazard below. Compass has its own real logo mark — `assets/compass-mark.png`, a cropped/cleaned export from the org's actual designed Compass logo (violet→coral gradient compass rose), used in the auth gate and the interior topbar — the interior UI otherwise intentionally keeps its original cyan/orange 1CloudHub-derived palette rather than a full re-theme. `assets/orbit-mark.png` (used in `website/`) and `assets/compass-mark.png` were both produced by cropping the real designed logo files with the org's brand colors preserved exactly — not hand-drawn approximations.
